...
package org.apache.royale.html.beads.models
{
	...
	/**
	 *  The ArrayListMultiSelectionModel class is a selection model for
	 *  a dataProvider that is an array list. Other selection models
	...
	public class ArrayListMultiSelectionModel extends EventDispatcher implements IMultiSelectionModel, IRollOverModel, IHasLabelField
	{
		/**
		 *  Constructor.
		 *  
		 ...
		 */
		public function ArrayListMultiSelectionModel()
		{
		}
		...
		public function set dataProvider(value:Object):void
		{
			...
			{
				var indices:Array = [];
				var length:int = (value as ArrayList).length;
				for (var i:int = 0; i < _selectedIndices.length; i++)
				{
					if (_selectedIndices[i] < length)
					{
						indices.push((value as ArrayList).getItemAt(_selectedIndices[i]));
					}
		...
		public function set selectedItems(value:Array):void
		{
			...
			_selectedIndices = [];
			var dp:ArrayList = _dataProvider as ArrayList;
			for (var i:int = 0; i < value.length; i++)
			{
				_selectedIndices.push(dp.getItemIndex(value[i]));
			}
			dispatchEvent(new Event("selectedItemsChanged"));			
			dispatchEvent(new Event("selectedIndicesChanged"));
		}
		...
		{
				var items:Array = [];
				var dp:ArrayList = dataProvider as ArrayList;
				for (var i:int = 0; i < _selectedIndices.length; i++)
				{
					items.push(dp.getItemAt(_selectedIndices[i]));
				}
