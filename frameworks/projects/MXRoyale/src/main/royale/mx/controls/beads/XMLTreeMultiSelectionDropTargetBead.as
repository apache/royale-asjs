...
package mx.controls.beads
{
	...
	/**
	 *  The XMLTreeMultiSelectionDragSourceBead handles drag source operaions special to XML data
	 *
		...
		private function handlingDragDropHandler(event:ValueEvent):void
		{
			...
			if (event.value)
			{
				...
				if (targetXML)
				{
					...
					var dragSources:Array = DragEvent.dragSource as Array;
					for (var i:int = 0; i < dragSources.length; i++)
					{
						if (parent && dragSources[i] is XML)
						{
