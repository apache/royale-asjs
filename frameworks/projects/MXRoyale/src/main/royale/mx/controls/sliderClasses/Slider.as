  ...
  /*  override */ protected function initializeAccessibility():void
    ...
    override public function styleChanged(styleProp:String):void
    {
        ...
        if (styleProp == "trackHighlightSkin" || anyStyle)
        {
            ...
            {
                innerSlider.removeChild(UIComponent(highlightTrack));
                highlightTrack = null;
            }
            trackHighlightChanged = true;
            invalidateProperties();
        ...
        if (styleProp == "trackSkin" || anyStyle)
        {
            if (track)
            {
                innerSlider.removeChild(UIComponent(track));  
                track = null;
                //createBackgroundTrack();
            }
        }
    ...
    /*    var n:int;
        ...
        if (directionChanged)
        {
            ...
            if (isHorizontal)
            {
                UIComponent(innerSlider).rotation = 0;
            }
            else
            {
                UIComponent(innerSlider).rotation = -90;
 ...
 /*  private function createBackgroundTrack():void
    {
        if (!track)
        {
            ...
            innerSlider.addChildAt(UIComponent(track),0); 
        }
    }  */
 ...
 /*   private function createHighlightTrack():void
    {
        ...
        if (!highlightTrack && showTrackHighlight)
        {
            ...
            innerSlider.addChildAt(UIComponent(highlightTrack),
                innerSlider.getChildIndex(UIComponent(track)) + 1);
        }
    }  */
...
/*  mx_internal function getValueFromX(xPos:Number):Number
 ...
 /*   private function track_mouseDownHandler(event:MouseEvent):void
     ...
     *  @private
     */
    private function thumb_focusInHandler(event:mx.events.FocusEvent):void
    {
        //dispatchEvent(event);
    }  
     ...
     *  @private
     */
    private function thumb_focusOutHandler(event:mx.events.FocusEvent):void
    {
        //dispatchEvent(event);
    }  
