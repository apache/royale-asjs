...
package mx.controls.beads.controllers
{
	 ...
	 */
	public class ADGItemRendererMouseController implements IBeadController
	{
		/**
		 *  constructor.
		 *  
		 ...
		 *  @productversion Royale 0.9
		 */
		public function ADGItemRendererMouseController()
		{
		}
		...
		protected function mouseDownHandler(event:MouseEvent):void
		{
			...
			if (target)
			{
				...
				var newEvent:ADGItemClickedEvent = new ADGItemClickedEvent("itemMouseDown");
				newEvent.data = target.data;
				newEvent.index = target.index;
				newEvent.originalDispatcher = event.target;
		...
		protected function mouseUpHandler(event:MouseEvent):void
		{
			...
			if (target)
			{				
				var newEvent:ADGItemClickedEvent = new ADGItemClickedEvent("itemClicked");
				newEvent.data = target.data;
				newEvent.index = target.index;
				newEvent.originalDispatcher = event.target;
		...
		protected function handleMouseUp(event:BrowserEvent):void
		{
			...
			if (target)
			{
				var newEvent:ADGItemClickedEvent = new ADGItemClickedEvent("itemMouseUp");
				newEvent.data = target.data;
				newEvent.index = target.index;
				newEvent.originalDispatcher = event.target;
				sendEvent(target,newEvent);
