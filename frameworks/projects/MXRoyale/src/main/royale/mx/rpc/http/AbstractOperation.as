...
package mx.rpc.http
{
	...
	COMPILE::JS
	{
		//import mx.rpc.xml.XMLDocument;
		//import mx.rpc.xml.XMLNode;
	...
	public class AbstractOperation extends mx.rpc.AbstractOperation
	{
		...
		public function sendBody(parameters:Object):AsyncToken
		{
			...
			if (ctype == CONTENT_TYPE_XML)
			{
				...
				else if (!(parameters is flash.xml.XMLNode) && !(parameters is XML))
				{
					if (xmlEncode != null)
					{
						...
						else if (!(funcEncoded is flash.xml.XMLNode))
						{
							token = new AsyncToken(null);
							msg = resourceManager.getString(
						...
						{
							paramsToSend = flash.xml.XMLNode(funcEncoded).toString();
						}
					}
					else
					{
						/*var encoder:SimpleXMLEncoder = new SimpleXMLEncoder(null);                    
						var xmlDoc:flash.xml.XMLDocument = new flash.xml.XMLDocument();
						
						//right now there is a wasted <encoded> wrapper tag
						//call.appendChild(encoder.encodeValue(parameters));
						var childNodes:Array = encoder.encodeValue(parameters, new QName(null, "encoded"), new flash.xml.XMLNode(1, "top")).childNodes.concat();                    
