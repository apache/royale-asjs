    ...
    */
    ...
    protected function getExistingValue(parent:*, propertyName:String):*
    {
        ...
        else
        {
            ...
            */
            var td:TypeDefinition = describeType(object);
            var props:Array = td.accessors;
            var n:int = props.length;
            for (var i:int = 0; i < n; i++)
            {
                var ad:AccessorDefinition = props[i];
                if (ad.name == propertyName)
                {
                    if (ad.access == "readwrite")
                    {
                        var propertyType:String = ad.type.qualifiedName;
                        var tempValue:* = parent[propertyName];
                        if (propertyType == "Object" || propertyType == "*" || TypeIterator.isIterable(tempValue))
                            existingValue = tempValue;
    ...
    protected function filterAttributes(attributes:XMLList):XMLList
    {
        ...
        for (var i:int = filtered.length() - 1; i >= 0; i--)
        {
            ...
            if (attr != null)
            {
                var name:QName = attr.name() as QName;
