...
package {
    ...
    public function isXMLName(str:String):Boolean {
        ...
        if (!l) return false;
        //check first char
        if (!isInTable(lettersTable, str.charCodeAt(0)) && str.charAt(0) != '_') return false;
        //check remaining chars
        for (var i:int=1;i<l;i++) {
            var charCode:uint = str.charCodeAt(i);
            if (isInTable(lettersTable,charCode)
                    || isInTable(digitsTable,charCode)
                    || charCode == 46 //'.'.charCodeAt(0) //@todo: check option for compiler inline of these literal patterns
                    || charCode == 45 //'-'.charCodeAt(0)
                    || charCode == 95 //'_'.charCodeAt(0)
                    || isInTable(combiningCharsTable, charCode)
                    || isInTable(extendersTable, charCode)) continue;
            return false;
        }
        return true;
    }
...
//@todo switch to Uint32Array
const lettersTable:Array = [
    0x0041, 0x005A,
    0x0061, 0x007A,
    0x00C0, 0x00D6,
    0x00D8, 0x00F6,
...
const combiningCharsTable:Array = [
    0x0300, 0x0345,
    0x0360, 0x0361,
    0x0483, 0x0486,
    0x0591, 0x05A1,
